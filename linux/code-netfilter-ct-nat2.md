...
* 容器A访问10.10.1.5:80，实际访问192.168.0.102:8080
* Host1实际收到的报文为（sip:10.10.1.2, sport:8888, dip:192.168.0.102, dport:8080）

### Host1 NF_INET_PRE_ROUTING点CT处理
		...
		struct sk_buff *skb)
{
	...
	ct = resolve_normal_ct(net, tmpl, skb, dataoff, pf, protonum,   //获取或创建ct对象，对象不存在，新建
		  ...
		  enum ip_conntrack_info *ctinfo)
{
	...
	h = __nf_conntrack_find_get(net, zone, &tuple, hash);  //根据tuple的hash值，找到内核中tuple对象，未找到
	if (!h) {
		h = init_conntrack(net, tmpl, &tuple, l3proto, l4proto,  //则新建nf_conn对象
				   skb, dataoff, hash);
		if (!h)
			return NULL;
		if (IS_ERR(h))
	...
	ct = nf_ct_tuplehash_to_ctrack(h);		//找到nf_conn对象（前一步骤新建的）
...
### Host1 NF_INET_PRE_ROUTING点NAT处理
					...
					struct nf_conn *ct))
{
	...
	case IP_CT_NEW:													    //进此分支
		...
		if (!nf_nat_initialized(ct, maniptype)) {	 //此时未执行过NAT操作，此条件满足
			...
			ret = do_chain(ops, skb, state, ct);      //iptable_nat_do_chain，未配置DNAT规则
			if (ret != NF_ACCEPT)
				return ret;

			if (nf_nat_initialized(ct, HOOK2MANIP(ops->hooknum)))  // 未执行DNAT，此条件不满足
				break;

			ret = nf_nat_alloc_null_binding(ct, ops->hooknum);	   //修改tuple
			if (ret != NF_ACCEPT)
				return ret;
		} else {
			   ...
			   struct sk_buff *skb)
{
	...
	if (ct->status & statusbit) {		          //由于未配置DNAT，此时ct未打上DNAT标记，不进此分支，直接返回立
...
__nf_nat_alloc_null_binding(struct nf_conn *ct, enum nf_nat_manip_type manip)
{
	...
	union nf_inet_addr ip =
		...
		ct->tuplehash[IP_CT_DIR_REPLY].tuple.src.u3);		//DNAT，IP为reply tuple的源IP，即192.168.0.102
		  ...
		  enum nf_nat_manip_type maniptype)
{
	...
	get_unique_tuple(&new_tuple, &curr_tuple, range, ct, maniptype);	//根据target的配置获取一个合适NAT转换之后的IP地址和端口信息，未修改

	if (!nf_ct_tuple_equal(&new_tuple, &curr_tuple)) {		//未执行DNAT，new_tuple和curr_tuple相同，此条件不成立
...
此阶段处理完成后，nf_conn对象的tuple没有修改（skb也没有被修改），信息如下：
...
### Host1 NF_INET_POST_ROUTING点NAT处理
					...
					struct nf_conn *ct))
{
	...
	nat = nf_ct_nat_ext_add(ct);	//找到NAT信息，此时ct已经包含nat信息（前一阶段添加）
	if (nat == NULL)
		return NF_ACCEPT;
	...
	case IP_CT_NEW:
		 ...
		 * or local packets.
		 */
		if (!nf_nat_initialized(ct, maniptype)) {		//当前在POSTROUTING，执行的是SNAT，此条件满足
			unsigned int ret;

			ret = do_chain(ops, skb, state, ct); //iptable_nat_do_chain，处理nat表中的snat规则，此时配置了snat规则，会最终修改reply tuple
			if (ret != NF_ACCEPT)
				return ret;

			if (nf_nat_initialized(ct, HOOK2MANIP(ops->hooknum)))  //因为配置了snat，所以此条件满足，跳出修改报文
				break;
			   ...
			   struct sk_buff *skb)
{
	...
	enum ip_conntrack_dir dir = CTINFO2DIR(ctinfo);
	unsigned long statusbit;
	enum nf_nat_manip_type mtype = HOOK2MANIP(hooknum);		//NF_INET_POST_ROUTING，执行SNAT

	if (mtype == NF_NAT_MANIP_SRC)
		statusbit = IPS_SRC_NAT;		//此分支
	else
		statusbit = IPS_DST_NAT;

	/* Invert if this is reply dir. */
	if (dir == IP_CT_DIR_REPLY)          //当前不是reply
		statusbit ^= IPS_NAT_MASK;

	/* Non-atomic: these bits don't change. */
	if (ct->status & statusbit) {                  //ct->status在iptable_nat_do_chain中最终会设置SNAT标记
		struct nf_conntrack_tuple target;

		/* We are aiming to look like inverse of other direction. */
		nf_ct_invert_tuplepr(&target, &ct->tuplehash[!dir].tuple);	   //使用reply报文的反转tuple，该tuple的源IP地址已经是SNAT后的地址
				  ...
				  enum nf_nat_manip_type maniptype)
{
	...
	if (maniptype == NF_NAT_MANIP_SRC) {
		...
		iph->saddr = target->src.u3.ip;		//此分支，修改报文的源IP为reply tuple反转tuple的源IP（即需要SNAT的地址）
	} else {
...
此阶段处理完成后，nf_conn对象的reply tuple信息被改变（skb也被修改），信息如下：
...
### Host1 NF_INET_POST_ROUTING点CT处理
...
报文转发给Host2节点，并最终发送响应报文（sip:192.168.0.102, sport:8080, dip:192.168.0.101, dport:1234）

Host1内核收到报文时，进入ip层收包后，会先进入NF_INET_PRE_ROUTING点
			   ...
			   struct sk_buff *skb)
{
	...
	/* Non-atomic: these bits don't change. */
	if (ct->status & statusbit) {				//由于CT已打上SNAT标记，所以此分支成立
		struct nf_conntrack_tuple target;
		...
		l4proto = __nf_nat_l4proto_find(target.src.l3num,
						target.dst.protonum);
		if (!l3proto->manip_pkt(skb, 0, l4proto, &target, mtype))		//修改报文的目的IP和目的端口
			return NF_DROP;
	}
	return NF_ACCEPT;
}
				  ...
				  enum nf_nat_manip_type maniptype)
{
	...
	if (maniptype == NF_NAT_MANIP_SRC) {
		...
		csum_replace4(&iph->check, iph->daddr, target->dst.u3.ip);
		iph->daddr = target->dst.u3.ip;		//reply报文，修改报文的目的IP为origin tuple的反转tuple的目的IP，即orign的源IP，即10.10.1.2
	}
	return true;
}
...
此阶段处理完成后，nf_conn对象的tuple信息无变化，skb的目的IP和目的MAC被修改了，信息如下：
...
内核继续IP层收包，发现报文的目的IP不是本节点，走ip_forward分支，最后进入到NF_INET_POST_ROUTING
			   ...
			   struct sk_buff *skb)
{
	...
	/* Non-atomic: these bits don't change. */
	if (ct->status & statusbit) {					//ct被标记为SNAT，此条件不满足，直接返回
		struct nf_conntrack_tuple target;
...
此阶段处理完成后，nf_conn对象的tuple信息无变化，skb也无修改，信息如下：
...
至此，发送节点的CT和NAT整个过程已经完成，整个过程比较绕，总结如下：

* CT的tuple只会修改一次，修改发生在origin方向的NAT操作时
* 在POSTROUTING点配置了SNAT，会在PREROUTING点执行DNAT
