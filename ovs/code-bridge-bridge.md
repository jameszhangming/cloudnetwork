...
open_dpif_backer(const char *type, struct dpif_backer **backerp)
{
    ...
    sset_init(&names);
    dp_enumerate_names(type, &names);    //遍历所有的dp
...
do_open(const char *name, const char *type, bool create, struct dpif **dpifp)
{
    ...
    error = registered_class->dpif_class->open(registered_class->dpif_class,     //内核态调用dpif_netlink_open
                                               name, create, &dpif);
    if (!error) {
                  ...
                  size_t n_revalidators)
{
    ...
    if (!udpif->handlers && !udpif->revalidators) {
        ...
        udpif_start_threads(udpif, n_handlers, n_revalidators);    //启动upcall线程和revalidate线程
    }
    ovsrcu_quiesce_end();
}
```
